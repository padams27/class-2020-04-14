...
ggplot(nes, aes(income, dvote)) + geom_jitter()
...
ggplot(nes, aes(income, dvote)) + geom_jitter(alpha = 0.4) + geom_smooth(method = lm) 
...
dvote is discrete - 0 or 1, and a linear regression will give points outside that so not good
...
model_1 <- glm(dvote ~ gender + income, binomial, data = nes)
...
s2 <- model_1 %>% 
  tidy(conf.int = T) %>%
  print(term, estimate, conf.high, conf.low)
...
An increse in income group by one unit leads to an increse in dvote of -0.288 / 4 
...
True value vs. if you ran it infinite times thats the 95 confidence 
...
income goes up, love for dems goes down
...
s3 <- model_1 %>% 
  augment(type.predict = "responce", data = nes)
...
the value on the regression line of x for a given y

Why does `.fitted` have the same value for every male with income = 3? (First, show that this is in fact true. Then explain why?)

```{r}

s3 %>% filter(income == 3, gender == "male")

```

its a value based off of the regression line so it has to be the same for each income groups
...
s2 %>% filter(term == 'gendermale') %>% qlogis(estimate, scale = 1)
...
ggplot(s3, aes(year, .resid)) + geom_point() + facet_wrap(~ gender) + geom_smooth(method = lm)
...
nes %>%
  ...
  summarize(perc_dem_win = mean(dvote),
